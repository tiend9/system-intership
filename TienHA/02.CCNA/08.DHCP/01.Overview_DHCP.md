...
DHCP là viết tắt của Dynamic Host Configuration Protocol, là một giao thức mạng được sử dụng để tự động cấp phát các thông số mạng (địa chỉ IP, địa chỉ Gateway, địa chỉ DNS...) thay vì cấu hình thủ công.
...
DHCP Client là thiết bị yêu cầu và nhận thông tin cấu hình mạng từ DHCP Server (máy tính, điện thoại, hoặc máy in...) đóng vai trò làm DHCP Client khi cần kết nối mạng. Chúng gửi yêu cầu để nhận địa chỉ IP và các thông số mạng như Subnet Mask, Gateway, và DNS Server. Sau khi nhận được thông tin từ DHCP Server, DHCP Client sử dụng các thông số này để kết nối và giao tiếp trong mạng.
...
- `Lỗi 1`: Không có IP → 169.254.x.x
→ DHCP không phản hồi

- `Lỗi 2`: Trùng IP
→ Có static IP trùng dải DHCP

- `Lỗi 3`: Lease quá dài
→ IP không được giải phóng

- `Lỗi 4`: Sai gateway/DNS
→ Có IP nhưng không ra Internet
