...
- `/etc/ssh/sshd_config` : File config cho phép máy mình kết nối ra ngoài (đóng vai trò như 1 client) → sửa hoặc kiểm tra ssh_config, muốn chỉnh sửa file thì dùng `sudo vi` or `sudo nano` và nhớ `sudo systemctl restart sshd` để lưu cấu hình sau khi thay đổi

- `/etc/ssh/ssh_config` : File config cho phép máy mình được SSH vào (đóng vai trò như 1 server) → chỉnh `sshd_config`, muốn chỉnh sửa file thì dùng `sudo vi` or `sudo nano` và nhớ `sudo systemctl restart ssh` để lưu cấu hình sau khi thay đổi

- `~/.ssh/config` : File cấu hình SSH Client **trên Linux**(ta check trên Ubuntu hay CentOS)
...
systemctl enable sshd (có hoặc không có dòng này cũng được)
```
