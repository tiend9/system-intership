...
**HTTP** hỗ trợ các cơ chế bộ nhớ đệm **(caching)**, **proxy**, và **web crawler**, giúp tối ưu lưu lượng mạng. Các tài nguyên **HTTP** được định danh bằng **URL (Uniform Resource Locator)**.
...
- **HTTP là giao thức connectionless** (kết nối không liên tục) : Khi client gửi request tới sever và sever phải gửi respone về thì client mới gửi request mới

- **HTTP là stateless (không trạng thái)** : Request hiện tại không biết những gì đã hoàn thành trong request trước đó. Điều này đòi hỏi các kỹ thuật bổ sung như cookie hoặc session để giữ lại trạng thái người dùng trong các thùng tương tác (container)
  
- **HTTP** là 1 trong các bộ giao thức TCP/IP và sử dụng cổng dịch vụ là 80
...
Như ta biết HTTP request chỉ là **stateless**, máy chủ không thể biết liệu các yêu cầu có phải từ cùng một trình duyệt web hay không.
...
- **Cookie xác thực** - Xác thực xem có phải đúng thiết bị và tài khoản đang truy cập không
...
| 405 | Method Not Allowed | Phương thức không được phép sử dụng |
...
| 503 | Service Unavailable | dịch vụ web không thể tực hiện |
| 504 | Gateway Timeout | Server trung gian không nhận được phản hồi |
...
## III. DIFFERENT BETWEEN HTTP AND HTTPS
