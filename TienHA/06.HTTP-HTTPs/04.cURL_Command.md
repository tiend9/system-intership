...
**Ví dụ**: Chẳng hạn ta muốn gửi dữ liệu JSON tới web site `http://mywebsite1.com`

```bash
curl -X POST -H "Content-Type: application/json" \-d '{"username":"tien","password":"123456"}' \https://mywebsite1.com
```
...
**Ví dụ**: Gửi nhiều tuỳ chọn Data cùng 1 lúc

```bash
curl -H "Content-Type: application/json" \-H "Authorization: Bearer 123abc" \-H "User-Agent: MyCustomClient/1.0" \https://api.example.com/users
```
