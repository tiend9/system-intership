...
![HTTP - HTTPs](./images/http_https_48.png)
...
# Ví dụ:
curl http://www.{google,tiktok,facebook}.com
```

![HTTP - HTTPs](./images/http_https_49.png)
...
### 3. Các tùy chọn hữu ích (Quan trọng nhất - master nó có thể tự xử lí ngon)
...
=> Có 1 số trang web tạo môi trường để ta test lệnh curl và thông dụng nhất ta sẽ dùng `https://httpbin.org/`

#### a. `curl -X [Method] [Data Option] [URL]` - Chỉ định phương thức (GET, POST, PUT, DELETE…)

**Ví dụ**: Chẳng hạn ta muốn gửi dữ liệu JSON tới web site `https://httpbin.org/post` Path`/post` (tài nguyên acc trong web)

```bash
curl -X POST -H "Content-Type: application/json" https://httpbin.org/post
```

![HTTP - HTTPs](./images/http_https_50.png)

#### b. `curl -H [Header Data/Option] [URL]` - Thêm header tùy chỉnh

**Ví dụ**: Gửi nhiều tuỳ chọn Data cùng 1 lúc tới website `https://mywebsite1.com`

```bash
curl -H "Content-Type: application/json"       #Gửi dự liệu dạng JSON
     -H "Authorization: Bearer 123abc"         #Gửi dữ liệu Authorization
     -H "User-Agent: MyCustomClient/1.0"       #Gửi dữ liệu User-Agent
     https://mywebsite1.com
```

![HTTP - HTTPs](./images/http_https_51.png)

=> các header option lên chỉ cách nhau 1 dòng

...

### 4.  Mẹo thêm

- Dùng `curl --help all` để xem danh sách đầy đủ tùy chọn.
