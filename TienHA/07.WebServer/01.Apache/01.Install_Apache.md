...
|**6. Ngắt SeLinux** | `sudo setenforce 0` | SeLinux chặn kết nối http|
|**7. Kiểm tra web server** | Truy cập `http://localhost/` hoặc `http://<IP_ubuntu>/` | Nếu thấy trang “Apache2 Ubuntu Default Page” → thành công |
|**8. Các file & thư mục quan trọng** |  |  |
...
|**9. Nạp lại cấu hình** | `sudo systemctl reload apache2` | Nạp lại cấu hình mà không dừng dịch vụ. |
|**10. Khởi động lại dịch vụ** | `sudo systemctl restart apache2` | Restart hoàn toàn Apache. |
|**11. Kiểm tra phiên bản** | `apache2 -v` | Xem phiên bản hiện tại *(VD: Apache/2.4.57 (Ubuntu))*. |

![WebServer](../01.Apache/Apache_images/WebServer_1.png)

![WebServer](../01.Apache/Apache_images/WebServer_2.png)
...
|**6. Ngắt SeLinux** | `sudo setenforce 0` | SeLinux chặn kết nối http|
| **7. Kiểm tra hoạt động** | Truy cập: `http://localhost`hoặc `http://<IP_CentOS>` | Nếu thấy trang **Apache Test Page**, nghĩa là cài thành công. |
| **8. Thư mục & file cấu hình** | `/var/www/html` and `/etc/httpd/conf/httpd.conf` and `/var/log/httpd/access_log` and `/var/log/httpd/error_log` | Thư mục web chính, file cấu hình và log. |
| **9. Kiểm tra & khởi động lại** | `sudo apachectl configtest` and `sudo systemctl restart httpd` | Kiểm tra cấu hình và khởi động lại dịch vụ nếu cần. |
...
![WebServer](../01.Apache/Apache_images/WebServer_3.png)

![WebServer](../01.Apache/Apache_images/WebServer_4.png)
