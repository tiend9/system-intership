...
#### Bước 1: Tải Apache về VM trên CentOS9 (Ở những phần trước)

#### Bước 2 : Tạo thư mục chứa website tĩnh

Giả sử bạn muốn tạo site tên `mywebsite`:

```bash
sudo mkdir -p /var/www/mywebsite
```

Phân quyền để dễ chỉnh sửa:
...
<html>
  ...
  <body>
    <h1>Hello from Apache on CentOS 9!</h1>
    <p>This is a static website served by Apache.</p>
  </body>
</html>
```
...
#### Bước 5: Tạo file cấu hình Virtual Host
...
#### Bước 6:Kiểm tra cấu hình Apache

```bash
sudo apachectl configtest
```

Nếu hiện `Syntax OK`-> Bạn có thể khởi động lại Apache.
...
#### Bước 8:Mở cổng tường lửa(firewalld)

Mặc định CentOS bật `firewalld`, nên cần cho phép HTTP/HTTPS:
...
#### Bước 9:Kiểm tra truy cập website
...
Nếu thấy website hiện ra như sau là đã thành công

![WebServer](./Apache_images/WebServer_16.png)
