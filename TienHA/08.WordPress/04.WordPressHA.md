...
Một trang web được triển khai thì có một thuật ngữ chúng ta cần quan tâm đó là **Traffic**(hay còn gọi là **lượt truy cập trang web**). Web càng lớn và càng phổ biến thì ta có lưu lượng **Traffic** càng nhiều. Và khi có lượng **Traffic** nhiều thì chúng ta sẽ cần phải quan tâm đến 1 khái niệm nữa đó là **WordpressHA** hay còn gọi là **WordpressHighAvailability**
...
## II. Kiến trúc và Thành phần trong WordPress HA
...
- **Khái niệm**: Một máy/**Instance**/Sever độc lập them gia vào hệ thống.
...
- **Tác dụng**:
  ...
  - Triển khai **WebNode** bằng cách chạy **Nginx/Apache**; **PHP-FPM**; **WPSourceCode** và **WebNode** sẽ có vai trò:  
...
### 3. Shared Storage và Object Storage
...
### 4. DB Nodes(MySQL Cluster: Primary-Replica/Galera)
...
## III. Phân tích một số thuật ngữ phổ biến trong kiến trúc WordPressHA

### 1. Ở tầng cao nhất (Host,Node,VM)

#### Phân tích các thuật ngữ Image - Instance - Snapshot - Container

- **Image(VMImage)**: Là khuôn mẫu(template) để tạo ra máy ảo
  ...
  - **Ví dụ**:
    ...
    - **AMI**(AWS)
    - **Custom Image**(OpenStack,GCP)
    - **Golden Image**(Trong doanh nghiệp)

=> 1 **Image** -> Có thể tạo ra nhiều **Instance**

- **Instance**: Là máy ảo đang chạy

  - **Đặc điểm**:

    - Được tạo ra từ Image
    - Có RAM, CPU, IP, DISK
    - Có thể Start/Stop, SSH vào và **install Plugin**

=> **Instance** là thứ trực tiếp xử lí request

- **Snapshot**: Bản chụp tại 1 thời điểm của Instance hay sao lưu trạng thái của Instance
  ...
  - **Đặc điểm**:
    ...
    - Có thể chụp **RootDisk, DataDisk, Cả hệ thống**(tuỳ Cloud)

=> Dùng để **Rollback** khi gặp lỗi là chủ yếu.

- **Container**: Là môi trường chạy ứng dụng nhẹ.
...
=> Giống **ImageVM** nhưng nhẹ hơn nhiều.

- **Đặt cả 4 thuật ngữ vào chung 1 sơ đồ**:
...
- **Trong WordPress HA có dùng Container không**?

  - Có hai mô hình mà **WP HA** dùng **Container**

    - **VM-based (phổ biến cho người mới)**

      - **Web Node** = **VM**
      - **PHP-FPM** chạy trực tiếp

    - **Container-based (cloud-native)**

      - **Web Node** = Pod(đơn vị triển khai nhỏ nhất của **Kurbernetes**)
      - **WordPress, PHP-FPM, Nginx** = Container
      - **Orchestrator**: Kubernetes

### 2. Ở tầng LoadBalancer

#### Tìm hiểu thuật ngữ Sticky Session và Shared Session

- **Sticky Session**: Là cơ chế điều phối request đảm bảo 1 người dùng luôn được phục vụ bởi cùng một **BackendNode**

- **Shared Session**: Là mô hình lưu trữ Session tập trung cho phép nhiều Node truy cập đồng thời, từ đó đảm bảo tính sẵn sàng cao và khả năng mở rộng của hệ thống.

  - **Đặc điểm**:

    - Đối với **Sticky Session**:

      - Thường lưu Sessions trong **RAM/LocalDisk** vì không có **Shared Local Store**
      - Thường được thực hiện bằng **IP Hash, Cookie, Header**
      - Dễ cấu hình (Không cần **Redis/DBSession**)
      - Ít thay đổi app
      - Nhưng **không true HA**(Node chết->user mất Session), load không đều và Scale kém

    - Đối với **Shared Session**:

      - **Session** thường được lưu ở **Shared Sessions Store(Redis), Memcached, Database, File Shared(NFS)**
      - Chuẩn **HA**(Node chết -> Session không mất)
      - **Scale** tốt
      - Nhưng cấu hình sẽ phức tạp hơn, cần thêm service(Redis)

=> **Kết luận**: Chuẩn **Production** thì ta lên dùng **Shared Session**, còn **Sticky Session** chỉ dùng trong DEMO,LAB

- **Ví dụ - Cấu hình cơ bản của 2 thằng**:

  - Đối với **Shared Session(Redis-PHP)**:
...
### 4. Ở tầng DBNode và StorageNode
...
- **Block Storage (Disk)**: Là kiểu lưu trữ dữ liệu dưới dạng các **block (khối)**, được **gắn (attach)** trực tiếp vào server như một ổ cứng.
...
- **File Storage/Shared Storage (Shared filesystem)**: Hệ thống lưu trữ cho phép nhiều server cùng truy cập và dùng chung dữ liệu (thường dưới dạng file system).
...
- **Object Storage (Cloud-native)**:là mô hình lưu trữ dữ liệu dưới dạng object, truy cập thông qua API (HTTP/REST), không gắn như ổ cứng truyền thống. Mỗi Object gồm data + metadata + ID
...
- **Khái niệm**: NFS(NetWork File System) là giao thức chia sẻ file qua mạng

  - Cho phép nhiều máy truy cập cùng 1 thư mục
  - Như thể thư mục đó nằm trong LocalDisk

- **NFS** hoạt động theo **mô hình**:

```text
Client  ←→  Network  ←→  NFS Server
```

- **Tác dụng NFS**: Dùng khi nhiều server cần dùng chung dữ liệu dạng file

  - Ví dụ:

    - Web server dùng chung thư mục /var/www/html
    - WordPress dùng chung wp-content/uploads

- **Cách hoạt động NFS**:

  - **Bước 1**: NFS Server export thư mục

  ```bash
  /export/wp-content
  ```

  - **Bước 2**: Client mount thư mục đó

  ```bash
  mount -t nfs nfs-server:/export/wp-content /var/www/html/wp-content
  ```

  - **Lúc này**:

    - WebSever ghi File
    - File nằm trên NFS Sever
    - WebSever khác thấy ngay

    => Như ổ cứng mạng

- **Đặc điểm quan trọng NFS**

  - NFS 100% hoạt động qua mạng, giao tiếp bằng bộ giao thức **TCP**, Dùng **Port 2049(NFSv4)** và phụ thuộc **Network Latency**, **NetworkBandwidth**.
  - Dễ triển khai, không cần tự sửa code, phù hợp WP, rẻ và phổ biến.
  - Nhưng là **SPOF**(chưa đáp ứng đủ tính HA), nếu muốn HA phải thêm **DRDB** or **Replication** or **use Managed NFS(EFS,FileStone)**

=> **Kết luận**: NFS chỉ đáp ứng đủ tính thay thế của HA chứ chưa khắc phục được điểm **SPOF** của **HA**.
