...
- **Trường hợp 1**: Hãy tạo một bảng `tickets` bao gồm một `priority` column có `ENUM` kiểu:
...
- **Th2**:Ta chèn 1 hàng mới vào bảng `tickets`
...
- **Th 3**: Chèn một hàng mới vào bảng `tickets` bằng cách sử dụng giá trị chỉ mục số thay vì các giá trị được xác định trước:
...
- Thứ tư, chèn nhiều hàng vào bảng `tickets`:
  
  - Chúng ta định nghĩa `priority` là một cột `NOT NULL`, khi bạn chèn một hàng mới mà không chỉ định giá trị cho prioritycột, MySQL sẽ sử dụng thành viên liệt kê đầu tiên làm giá trị mặc định. Ví dụ:
