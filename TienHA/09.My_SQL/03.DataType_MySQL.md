...
- `ENUM` là **iểu dữ liệu dạng chuỗi** trong MySQL, cho phép **một cột** chỉ **lưu trữ một giá trị duy nhất** được chọn **từ danh sách các giá trị cố định đã khai báo** khi tạo bảng.
...
- Hãy tạo một `TABLE` bao gồm một `priority` column có `ENUM` kiểu:
