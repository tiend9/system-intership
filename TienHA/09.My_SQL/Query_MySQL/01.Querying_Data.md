...
- Khi dùng `SELECT` để truy vấn dữ liệu từ 1 hàng or cột, và muốn thứ tự trong hàng or cột được sắp xếp ta dùng `ORDER BY` trong truy vấn `SELECT`. Cú pháp chung:
...
- Trong đó:
  ...
  - Có thể sắp xếp 1 cột tăng dần ,1 cột giảm dần(Sắp xếp đa dạng)

#### Example `ORDER BY`

- Sử dụng `ORDER BY` để sắp xếp danh sách khách hàng theo họ và theo thứ tự tăng dần bảng chữ cái dùng bảng `customers` trong sample DB.
...
![Query_MySQL](./Query_images/Queryimages_68.png)

- Theo thứ tự giảm dần:

![Query_MySQL](./Query_images/Queryimages_69.png)

#### Sử dụng `ORDER BY` để sắp xếp 1 biểu thức

- Xét bảng `orderDetails` trong sample DB.

- Chọn các dòng đơn hàng từ bảng này, tính `subtotal = quantity Ordered * price Each` và sắp xếp theo biểu thức `subtotal`:
...
![Query_MySQL](./Query_images/Queryimages_70.png)

#### Sử dụng `ORDER BY` để Sort theo 1 danh sách tuỳ chỉnh

- Hàm FIELD() trả về vị trí (chỉ số) của 1 giá trị trong danh sách các giá trị. Cú pháp chung:
...
- Trong đó:
  ...
  - Hàm `FIELD` trả về vị trí của `value` trong danh sách `value1`, `value2`,...
  - Nếu không tồn tại trong danh sách thì trả về `0`

- Ví dụ sắp xếp các đơn hàng theo trạng thái với thứ tự tuỳ chỉnh:
...
![Query_MySQL](./Query_images/Queryimages_71.png)

#### `ORDER BY` với `NULL`

-`NULL` xuất hiện trước các giá trị không phải `NULL`

- Khi dùng `ORDER BY` với tuỳ chọn `ASC`, các giá trị `NULL` sẽ xuất hiện đầu tiên trong kết quả

- ví dụ: Truy vấn sắp xếp nhân viên theo cột `reportsTo`:

![Query_MySQL](./Query_images/Queryimages_72.png)
...
- **Truy vấn con** (SubQuery) là truy vấn được lồng ghép bên trong một truy vấn khác, ví dụ như `SELECT`, `INSERT`, `UPDATE` và `DELETE`

- Một truy vấn con có thể được lồng ghép bên trong 1 truy vấn con khác.

- Truy vấn con có thể sử dụng ở bất kì đâu miễn là biểu thức đó sử dụng được đựng trong dấu ngoặc đơn.

#### Example subQuery

- Ví dụ, truy vấn sau sử dụng truy vấn con để trả về danh sách nhân viên làm việc tại các văn phòng đặt tại Hoa Kỳ :
...
![Query_MySQL](./Query_images/Queryimages_73.png)

- Trong đó:

  - Truy vấn con trả về tất cả mã văn phòng của các văn phòng đặt tại Hoa Kỳ.
  - Truy vấn bên ngoài chọn họ và tên của các nhân viên làm việc tại các văn phòng có mã văn phòng nằm trong tập kết quả do truy vấn con trả về.
  - Khi thực thi truy vấn, MySQL sẽ đánh giá truy vấn con trước và sử dụng kết quả của truy vấn con cho truy vấn bên ngoài.

#### Sử dụng truy vấn con với toán tử `IN` và `NOT IN`

- Tìm khách hàng chưa đặt đơn hàng nào từ bảng `customers`
...
![Query_MySQL](./Query_images/Queryimages_74.png)
...
![Query_MySQL](./Query_images/Queryimages_75.png)

#### SubQuery với điều kiện `EXISTS` và `NOT EXISTS`

- SubQuery với điều kiện `EXISTS` và `NOT EXISTS`, truy vấn con sẽ trả về giá trị Boolean là `TRUE` nếu có ít nhất là 1 dòng và trả về `FALSE` nếu không có dòng nào

- Tìm đơn hàng có ít nhất 1 đơn hàng > 60000.
...
![Query_MySQL](./Query_images/Queryimages_76.png)

### Derived Tables

- **Bảng dẫn xuất** (Derived Tables) là 1 bảng ảo được trả về từ câu lệnh `SELECT`(tương tự như **bảng tạm thời**)

- Thuật ngữ `subQuery` và `Derived Tables` na ná nhau do `Derived Tables` được tạo ra khi một truy vấn con độc lập được sử dụng trong mệnh đề `FROM` của 1 lệnh `SELECT`
  
  - `Lưu ý`: truy vấn con đọc lập là 1 truy vấn con có thể thực thi độc lập với truy vấn bên ngoài. Ngoài ra, **bảng dẫn xuất bắt buộc phải có bí danh**.

- Cú pháp chung:

![Query_MySQL](./Query_images/Queryimages_77.png)

#### Example về Derived Tables

- Truy vấn sau đây lấy ra năm sản phẩm bán chạy nhất năm 2003 từ các bảng `orders` và `orderdetails` trong sample DB :

![Query_MySQL](./Query_images/Queryimages_78.png)
...
-> Kết quả truy xuất ra bảng `productCode`:

![Query_MySQL](./Query_images/Queryimages_79.png)

Bạn có thể sử dụng kết quả của truy vấn này làm bảng dẫn xuất và kết hợp nó với bảng `products` chính như sau:

![Query_MySQL](./Query_images/Queryimages_80.png)
...
![Query_MySQL](./Query_images/Queryimages_81.png)

### Toán tử `EXISTS` / `NOT EXISTS`

- `EXISTS` là 1 toán tử Boolean, thường được sử dụng để kiểm tra sự tồn tại của các hàng (rows) được trả về bởi 1 SubQuery. Cú pháp chung:
...
![Query_MySQL](./Query_images/Queryimages_82.png)
...
![Query_MySQL](./Query_images/Queryimages_83.png)
...
![Query_MySQL](./Query_images/Queryimages_84.png)
...
![Query_MySQL](./Query_images/Queryimages_85.png)

## 5. Set Operations – Toán tử tập hợp

### `EXCEPT`- Ngoại trừ

- Toán tử `EXCEPT` cho phép bạn truy xuất các hàng từ 1 truy vấn mà không xuất hiện trong 1 truy vấn khác. Cú pháp chung:
...
- Trong đó:
  ...
  - EXCEPT sẽ so sánh kết quả của Query1 với tập kết quả của Query2 và trả về kết quả những hàng xuất hiện trong Query1 nhưng không xuất hiện trong Query2.
  - Mặc định, toán tử `EXCEPT`sử dụng tuỳ chọn `DISTINCT` nếu ta không chỉ định `ALL`.
  - Để sử dụng toán tử `EXCEPT`, Query1 và Query2 cần tuân thủ các quy tắc sau:
    - Thứ tự và số lượng cột trong danh sách `SELECT`của 2 truy vấn phải giống nhau.
    - Kiểu dữ liệu của các cột tương ứng phải tương thích.
    - Toán tử `EXCEPT` trả về 1 một tập truy vấn với tên cột được lấy từ tên cột của truy vấn đầu tiên (Query1)

#### Simple Example `EXCEPT`
...
![Query_MySQL](./Query_images/Queryimages_86.png)

#### Reality Example `EXCEPT`

- Tìm các tên riêng(**firstName**) xuất hiện trong bảng `employees` nhưng không xuất hiện trong `customers`.

![Query_MySQL](./Query_images/Queryimages_87.png)

#### `EXCEPT` với `ORDER BY`

- Để sắp xếp tập kết quả được trả về bởi toán tử `EXCEPT`, bạn dùng mệnh đề `ORDER BY`

![Query_MySQL](./Query_images/Queryimages_88.png)

#### `EXCEPT` với `ALL`

![Query_MySQL](./Query_images/Queryimages_89.png)
...
![Query_MySQL](./Query_images/Queryimages_90.png)

- Sơ đồ sau minh hoạ rõ về toán tử này:

![Query_MySQL](./Query_images/Queryimages_91.png)

## Thứ tự các mệnh đề chuẩn nhất

![Query_MySQL](./Query_images/Queryimages_92.png)
