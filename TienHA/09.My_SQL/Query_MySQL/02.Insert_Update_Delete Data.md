...
![Query_MySQL](./Query_images/Queryimages_114.png)

### `UPDATE JOIN`

#### Introduction `Update JOIN`

- Chúng ta sử dụng phép nối `JOIN` với mệnh đề `UPDATE` để cập nhật các hàng trong 1 bảng dựa trên các giá trị từ một bảng khác.

- Câu lệnh này hữu ích khi bạn cần sửa dữ liệu trên nhiều bảng liên quan.
...
- Trong đó:
  ...
  - Đầu tiên, chỉ định bảng mà bạn muốn cập nhật sau từ khoá `UPDATE` ( `T1`- table1 )
  - Thứ hai, sử dụng mệnh đề `INNER JOIN` hoặc `LEFT JOIN` và 1 vị ngữ nối. Và mênhh đề `JOIN` này phải xuất hiện ngay sau mệnh đề `UPDATE` kia
  - Gán các giá trị mới cho các cột trong bảng `T1` mà bạn muốn cập nhật dữ liệu.
  - Cuối cùng, hãy chỉ định điều kiện trong mệnh đề `WHERE` để lọc các hàng cần cập nhật.

- Ta có thể viết mệnh đề UPDATE JOIN theo 2 cách:
...
#### Introduction `Delete Join`

MySQL cho phép xoá dữ liệu từ một hoặc nhiều bảng dựa trên mối quan hệ giữa bảng bằng `INNER JOIN` hoặc `LEFT JOIN` trong câu lệnh DELETE.
...
```
  ...
  -> Xoá các hàng trong bảng trái (T1) mà không có bản ghi tương ứng trong bảng phải (T2).  
  -> Chỉ liệt kê bảng cần xoá sau DELETE, không xoá cả 2 bảng như INNER JOIN.

### ON DELETE CASCADE

#### Introduction

- `ON DELETE CASCADE` cho phép xoá tự động các bảng ghi ở bảng con khi bản ghi tương ứng ở bảng cha bị xoá.
- Đây là cách hiệu quả để quản lí dữ liệu quan hệ 1:N mà không cần xoá thủ công ở từng bảng con.
...
![Query_MySQL](./Query_images/Queryimages_115.png)

- **Bước 2**: Tạo bảng rooms(bảng con) với khoá ngoại `ON DELETE CASCADE`
...
![Query_MySQL](./Query_images/Queryimages_116.png)
...
![Query_MySQL](./Query_images/Queryimages_117.png)
...
![Query_MySQL](./Query_images/Queryimages_118.png)

- **Bước 5**: Xoá 1 toà nhà:
...
-> Toà nhà số 2 bị xoá.
-> Các phòng thuộc toà nhà số 2 cũng bị xoá

![Query_MySQL](./Query_images/Queryimages_119.png)

### `TRUNCATE TABLE`

- Câu lệnh `TRUNCATE TABLE` cho phép bạn xoá tất cả các hàng khỏi 1 bảng. Cú pháp chung:
...
- Trong đó:
  ...
  - Ta chỉ định bảng mà ta muốn xoá tất cả các hàng sau từ khoá `TRUNCATE TABLE`
