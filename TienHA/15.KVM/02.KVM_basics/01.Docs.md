...
```bash
     ...
     +--------------------+
         ...
         +------------+
               ↓
     +--------------------+
     |     QEMU + KVM     |     <- Thực thi VM (QEMU mô phỏng, KVM tăng tốc bằng phần cứng). Hay còn gọi VM Layer
     +--------------------+
               ↓
         +----------+
         |  Kernel  |     <- Linux Host Kernel có mô-đun KVM. Hay còn gọi lớp này là Kernel Support Layer
         +----------+
               ↓
         +----------+
         | Hardware |
