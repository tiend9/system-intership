...
```bash
virsh detach-interface --domain tien9a --type network --mac 52:54:00:0c:b6:e6 --persistent
```

### 6. Attach NIC mới (Isolated)

```bash
virsh attach-interface --domain tien9a --type network --source isolated --model virtio --persistent
```
